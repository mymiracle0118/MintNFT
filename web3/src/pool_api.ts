...
export let programId = new PublicKey('UcoVBEvJ5XApSp9qZTSuGYxgBdJbSzoauxVRRrjsCqW')
...
export async function mintNft(
    ...
    let provider = new anchor.Provider(conn,wallet,anchor.Provider.defaultOptions())
    const program = new anchor.Program(idl,programId,provider)
    let client = (await PublicKey.findProgramAddress([programId.toBuffer(),pool.toBuffer(),owner.publicKey.toBuffer()],programId))[0]
...
export async function setMaxPrice(
    ...
    let provider = new anchor.Provider(conn,wallet,anchor.Provider.defaultOptions())
    const program = new anchor.Program(idl,programId,provider)
    let metadata_extended = (await PublicKey.findProgramAddress([mint.toBuffer(),pool.toBuffer(),programId.toBuffer()],programId))[0]
    try {
        await program.rpc.setMaxPrice(
